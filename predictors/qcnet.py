...
class QCNet(pl.LightningModule):
    """QCNet 轨迹预测的 Lightning 封装。

    模块主要包含三个阶段：
    - train/val：优化回归损失与多模态分类损失。
    - val：计算并记录 Argoverse 风格评测指标。
    - test：按场景汇总多模态预测并导出提交文件。
    """
    ...
    def training_step(self,
                      data,
                      batch_idx):
        # Batch 场景下，av_index 在每个子图内是局部索引，需要按 ptr 做偏移。
        if isinstance(data, Batch):
            data['agent']['av_index'] += data['agent']['ptr'][:-1]
        # reg_mask 覆盖全部未来步；cls_mask 仅使用最后一步。
        ...
        # 在有效时间步上，按 proposal 与 GT 的 L2 距离选择监督 mode。
        l2_norm = (torch.norm(traj_propose[..., :self.output_dim] -
                              gt[..., :self.output_dim].unsqueeze(1), p=2, dim=-1) * reg_mask.unsqueeze(1)).sum(dim=-1)
        best_mode = l2_norm.argmin(dim=-1)
        traj_propose_best = traj_propose[torch.arange(traj_propose.size(0)), best_mode]
        traj_refine_best = traj_refine[torch.arange(traj_refine.size(0)), best_mode]
        # 按每个时间步的有效标签数量做归一化，避免掩码稀疏导致偏置。
    ...
    def validation_step(self,
        ...
        if not self.output_head:
            # 若模型未直接输出航向角，则根据相邻位置差分恢复 heading。
    ...
    def configure_optimizers(self):
        # 构建参数分组：仅对白名单模块的 weight 使用 weight decay。
        decay = set()
        no_decay = set()
